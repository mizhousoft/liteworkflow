...
public class ProcessInstanceServiceImpl extends AccessService implements ProcessInstanceService
{
	...
	{
		return processInstanceEntityService.getByInstanceId(instanceId);
	}
	        ...
	        String parentNodeName)
	{
		...
		{
			if (!StringUtils.isBlank(model.getExpireTime()))
			{
				LocalDateTime expireTime = (LocalDateTime) args.get(model.getExpireTime());
	...
	{
		ProcessInstance instance = processInstanceEntityService.getByInstanceId(instanceId);
	...
	public void createCCInstance(String instanceId, String creator, String... actorIds)
	{
		for (String actorId : actorIds)
		{
			...
			ccInstance.setCreateTime(LocalDateTime.now());
			ccProcessInstanceEntityService.addEntity(ccInstance);
		}
	}
	...
	{
		List<CCProcessInstance> ccInstances = ccProcessInstanceEntityService.queryList(instanceId, actorIds);
		for (CCProcessInstance ccInstance : ccInstances)
		{
			ccInstance.setStatus(Constants.STATE_FINISH);
			ccInstance.setFinishTime(LocalDateTime.now());
			ccProcessInstanceEntityService.modifyEntity(ccInstance);
		}
	}
	...
	{
		List<CCProcessInstance> ccInstances = ccProcessInstanceEntityService.queryList(instanceId, actorId);
		for (CCProcessInstance ccInstance : ccInstances)
		{
			ccProcessInstanceEntityService.deleteEntity(ccInstance);
		}
	}
	...
	public void cascadeRemove(String id)
	{
		...
		List<CCProcessInstance> ccInstances = ccProcessInstanceEntityService.queryList(id);
		for (CCProcessInstance ccInstance : ccInstances)
		{
			ccProcessInstanceEntityService.deleteEntity(ccInstance);
		}

		ProcessInstance instance = processInstanceEntityService.getByInstanceId(id);
