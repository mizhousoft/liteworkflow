...
public abstract class AbstractDBAccess implements DBAccess
{
	...
	public void saveProcess(Process process)
	{
		...
		else
		{
			...
			int[] type = new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.VARCHAR, Types.INTEGER,
	...
	public void updateProcess(Process process)
	{
		...
		{
			Object[] args = new Object[] { process.getName(), process.getDisplayName(), process.getState(), process.getInstanceUrl(),
			        process.getCreateTime(), process.getCreator(), process.getId() };
	...
	public void saveTask(Task task)
	{
		...
		else
		{
			Object[] args = new Object[] { task.getId(), task.getOrderId(), task.getTaskName(), task.getDisplayName(), task.getTaskType(),
			        task.getPerformType(), task.getOperator(), task.getCreateTime(), task.getFinishTime(), task.getExpireTime(),
			        task.getActionUrl(), task.getParentTaskId(), task.getVariable(), task.getVersion() };
			int[] type = new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.INTEGER,
	...
	public void saveOrder(Order order)
	{
		...
		{
			Object[] args = new Object[] { order.getId(), order.getProcessId(), order.getCreator(), order.getCreateTime(),
			        order.getParentId(), order.getParentNodeName(), order.getExpireTime(), order.getLastUpdateTime(),
			        order.getLastUpdator(), order.getOrderNo(), order.getVariable(), order.getVersion() };
	...
	public void saveCCOrder(CCOrder ccorder)
	{
		...
		else
		{
			...
			saveOrUpdate(buildMap(CCORDER_INSERT, new Object[] { ccorder.getOrderId(), ccorder.getActorId(), ccorder.getCreator(),
			        ccorder.getCreateTime(), ccorder.getStatus() }, type));
		}
	}
	...
	public void updateTask(Task task)
	{
		...
		{
			Object[] args = new Object[] { task.getFinishTime(), task.getOperator(), task.getVariable(), task.getExpireTime(),
			        task.getActionUrl(), task.getId(), task.getVersion() };
			int[] type = new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
			        Types.INTEGER };
	...
	public void updateOrder(Order order)
	{
		...
		{
			Object[] args = new Object[] { order.getLastUpdator(), order.getLastUpdateTime(), order.getVariable(), order.getExpireTime(),
	...
	public void saveHistory(HistoryOrder order)
	{
		...
		else
		{
			Object[] args = new Object[] { order.getId(), order.getProcessId(), order.getOrderState(), order.getCreator(),
			        order.getCreateTime(), order.getEndTime(), order.getParentId(), order.getExpireTime(), order.getOrderNo(),
			        order.getVariable() };
			int[] type = new int[] { Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
		...
		else
		{
			...
			if (task.getActorIds() != null)
			{
				for (String actorId : task.getActorIds())
				{
					if (StringHelper.isEmpty(actorId))
						continue;
					saveOrUpdate(buildMap(TASK_ACTOR_HISTORY_INSERT, new Object[] { task.getId(), actorId },
	...
	public void saveSurrogate(Surrogate surrogate)
	{
		...
		{
			Object[] args = new Object[] { surrogate.getId(), surrogate.getProcessName(), surrogate.getOperator(), surrogate.getSurrogate(),
			        surrogate.getOdate(), surrogate.getSdate(), surrogate.getEdate(), surrogate.getState() };
			int[] type = new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
	...
	public void updateSurrogate(Surrogate surrogate)
	{
		...
		{
			Object[] args = new Object[] { surrogate.getProcessName(), surrogate.getSurrogate(), surrogate.getOdate(), surrogate.getSdate(),
			        surrogate.getEdate(), surrogate.getState(), surrogate.getId() };
			int[] type = new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.INTEGER,
			        Types.VARCHAR };
	...
	{
		String sql = QUERY_TASK
		        + " where parent_task_id in ( select ht.id from wf_hist_task ht where ht.order_id=? and ht.task_name=? and ht.parent_task_id=? )";
