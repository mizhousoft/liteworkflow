...
public class QuartzScheduler implements IScheduler
{
	...
	public void schedule(JobEntity entity)
	{
		...
		JobDetail job = JobBuilder.newJob(jobClazz).usingJobData(data).withIdentity(jobId, GROUP).build();
		Trigger trigger = null;
		TriggerBuilder<Trigger> builder = TriggerBuilder.newTrigger().withIdentity(StringHelper.getPrimaryKey(), GROUP)
		...
		if (jobClazz == ReminderJob.class && entity.getPeriod() > 0)
		{
			...
			if (count <= 0)
				count = 1;
			builder.withSchedule(SimpleScheduleBuilder.repeatMinutelyForTotalCount(count, entity.getPeriod()));
			if (isUseCalendar)
			{
